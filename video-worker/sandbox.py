...
#: Manim classes this image ships the code for but cannot render.
#:
#: **A capability rule, not a security rule**, which is why it is a separate list from
#: ``FORBIDDEN_NAMES`` with a separate message. Every name here is a mobject that builds itself by
#: shelling out to ``latex``, and the Dockerfile deliberately does not install TeX Live — two
#: gigabytes for a handful of formulas. The classes still import, so nothing fails until the scene
#: is half a minute into rendering and then dies inside ``tex_mobject.py`` with a traceback the fix
#: loop cannot do anything useful with.
#:
#: Stopping it here instead turns a wasted render into an instant rejection carrying the one
#: sentence that fixes it. Measured before writing this: the first animated scene this feature ever
#: planned reached for ``MathTex`` and lost its animation to exactly that traceback.
#:
#: ``Matrix``, ``Integer`` and ``DecimalNumber`` are on the list for a reason that is easy to miss —
#: their *brackets* and *digits* are LaTeX, so they fail without the code ever mentioning a formula.
#: Each maps to the line that replaces it, because a rejection the model cannot act on costs the
#: same two attempts as no rejection at all. Measured: told only "use Text instead", the fix loop
#: came back with the same `Integer(0)` it had just been rejected for.
LATEX_NAMES: dict[str, str] = {
    "Tex": 'Text("the words")',
    "MathTex": 'Text("O(n log n)")',
    "SingleStringMathTex": 'Text("O(n log n)")',
    "BulletedList": "a VGroup of Text lines arranged with .arrange(DOWN)",
    "Title": 'Text("the title").to_edge(UP)',
    "Matrix": "a VGroup of Text cells arranged with .arrange_in_grid()",
    "IntegerMatrix": "a VGroup of Text cells arranged with .arrange_in_grid()",
    "DecimalMatrix": "a VGroup of Text cells arranged with .arrange_in_grid()",
    "MobjectMatrix": "a VGroup arranged with .arrange_in_grid()",
    "DecimalNumber": 'Text("3.5")',
    "Integer": 'Text("5")',
    "Variable": 'Text("x = 5")',
}
...
def _check_node(node: ast.AST) -> None:
    ...
    if isinstance(node, ast.Name):
        ...
        if node.id in LATEX_NAMES:
            # The sentence carries the replacement, not just the refusal. It is the one rejection
            # the model can always act on, and it only works if it says what to write instead.
            raise Rejection(
                f"`{node.id}` needs a LaTeX installation this renderer does not have "
                f"(line {getattr(node, 'lineno', 0)}). Replace every `{node.id}(...)` with "
                f"{LATEX_NAMES[node.id]}, and write mathematics the way it is read aloud."
            )
