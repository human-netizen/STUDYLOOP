...
def test_latex_mobjects_are_rejected_with_the_fix_in_the_message():
    """Also not a security rule. The image has no TeX, so these classes cannot draw anything.

    Left to run, `MathTex` fails deep inside `tex_mobject.py` after Manim has already spent the
    scene's time budget, and the fix loop is handed a traceback instead of an instruction. The
    assertion on the message is the point of the test: a rejection the model cannot act on costs
    the same two attempts as no rejection at all.
    """

    verdict = sandbox.check(scene('formula = MathTex("O(n)")'))
    assert not verdict.ok
    assert verdict.layer == sandbox.LAYER_REJECTED
    assert "LaTeX" in verdict.detail
    assert "Text" in verdict.detail


def test_a_matrix_is_rejected_even_though_it_looks_like_shapes():
    """`Matrix` draws its brackets with LaTeX, so it fails without a formula anywhere in sight."""

    verdict = sandbox.check(scene("grid = Matrix([[1, 2], [3, 4]])"))
    assert not verdict.ok
    assert verdict.layer == sandbox.LAYER_REJECTED


def test_text_is_still_allowed():
    """The list is narrow on purpose: everything drawable stays drawable."""

    verdict = sandbox.check(scene('label = Text("O(n log n)").scale(0.8)\nself.add(label)'))
    assert verdict.ok
    assert verdict.layer == sandbox.LAYER_ALLOWED
