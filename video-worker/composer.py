...
def _scene_part(visual: Path, audio: Path, spoken: float, target: Path,
    ...
    if visual.suffix == ".png":
        stages = _stages(visual)
        if stages:
            return _revealed_part(stages, audio, spoken, target, width, height, fps)
...
def _stages(slide: Path) -> list[Path]:
    """The partial slides the renderer wrote beside the finished one, in order.

    Empty for a scene with one bullet, and empty for the sources card, both of which have nothing
    to reveal — the caller then falls back to holding the single image.
    """

    found = sorted(slide.parent.glob(f"{slide.stem}-*.png"))
    return found if len(found) > 1 else []


def _revealed_part(stages: list[Path], audio: Path, spoken: float, target: Path,
                   width: int, height: int, fps: int) -> bool:
    """A slide that fills in, one bullet at a time, across its own narration.

    **A still image held for thirty-five seconds is what makes a generated video feel like a
    slideshow**, and thirty-five seconds is what the script writer produces for a scene. There is
    nothing to add to the picture that would be true — the bullets are all there is — so the change
    is *when* they arrive rather than what they are.

    The concat demuxer rather than a filter graph with one input per stage: the durations are the
    whole of what this needs to say, the last entry is repeated without a duration so its frame is
    held to the end, and `-t` from the narration decides the length exactly as it does for a still.
    """

    share = spoken / len(stages)
    listing = target.parent / "stages.txt"
    lines = "".join(f"file '{stage.as_posix()}'\nduration {share:.3f}\n" for stage in stages)
    listing.write_text(lines + f"file '{stages[-1].as_posix()}'\n", encoding="utf-8")

    return _run([
        "ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(listing), "-i", str(audio),
        "-t", f"{spoken:.3f}",
        "-vf", f"scale={width}:{height},fps={fps},format=yuv420p",
        *_VIDEO_ARGS, *_AUDIO_ARGS,
        "-video_track_timescale", "90000",
